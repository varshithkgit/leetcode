...
function UpdatePrblm(){
  ...
  useEffect(()=>{
  const getPrblmByIdForUpd= async () => {
    try{
        setLoading(true);
        const {data}= await axiosClient.get(`prblm/getPrblmByIdForUpd/${id}`);
        setPrblm(data.prblm);
        setErr("");
    }catch(e){
        setErr("Failed to update data")
    }finally{
        setLoading(false);
    }
  }
  ...
  useEffect(()=>{
    console.log(prblm?.Aprouch);
    reset({ 
        title:prblm?.title,
        description:prblm?.description,
        difficulty:prblm?.difficulty,
        tags:prblm?.tags,
        startCode: prblm?.startCode,
        referenceSolution: prblm?.referenceSolution,
        visibleTestCases:prblm?.visibleTestCases,
        hiddenTestCases:prblm?.hiddenTestCases,
        Aprouch:[
                  {concept:prblm?.Aprouch[0]?.concept?.join(","),steps:prblm?.Aprouch[0]?.steps?.join(","),complexities:prblm?.Aprouch[0]?.complexities?.join(","),name:"Brute Force"},
                  {concept:prblm?.Aprouch[1]?.concept?.join(","),steps:prblm?.Aprouch[1]?.steps?.join(","),complexities:prblm?.Aprouch[0]?.complexities?.join(","),name:"Better Aprouch"},
                  {concept:prblm?.Aprouch[2]?.concept?.join(","),steps:prblm?.Aprouch[2]?.steps?.join(","),complexities:prblm?.Aprouch[0]?.complexities?.join(","),name:"Optimized Aprouch"}
        ]
     });
  },[prblm]);
  
  const updateHandler =  async (prblmData)=>{
       setLoading(true);
       try{
          ...
          await axiosClient.put(`/prblm/update/${id}`,{...prblmData,Aprouch:prblmArray});
          alert("Problem updates successfully.");
          setLoading(false);
...
<div className="min-h-screen bg-[#121212] text-[#f5f5f5] py-10 px-4">
  ...
  >
    <h1 className="text-4xl font-bold text-center">Update Problem</h1>
